...
#if defined(NDEBUG)
# error "Aryacoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Aryacoin Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s AYA additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight == 1) return COIN * 200000000;
...
bool DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    {
       // Dpow Enabled
         // printf("NOTE(main.cpp) -> DPOW Enabled");
        komodo_disconnect((CBlockIndex *)pindex,(CBlock *)&block);
    
    }
    else
    {// printf("NOTE(main.cpp) -> DPOW Disabled");
        // Dpow Disabled 
    }
...
void ThreadScriptCheck() {
    RenameThread("aryacoin-scriptch");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Aryacoin: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Aryacoin: (mainnet >= 710000, regtest and testnet uses supermajority)
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Aryacoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
