...
extern char ASSETCHAINS_SYMBOL[65]; // defined in validation.cpp
...
// 3: ENDS: Sunday, June 14th, 2020 03:00:00 PM UTC
// 4: ENDS: Monday, June 14th, 2021 (03:00:00 PM UTC)
...
// Fourth season ends at around June 14th, 2021 (03:00:00 PM UTC) ,AYA block number: 773000
// Fifth season end is unknown so is set to 999999999
static const int32_t KMD_SEASON_HEIGHTS[NUM_KMD_SEASONS] = {0, 335000, 398750, 773000, 999999999};
...
int32_t gettxout_scriptPubKey(int32_t height,uint8_t *scriptPubKey,int32_t maxsize,uint256 txid,int32_t n)
{
    ...
    if ( KOMODO_TXINDEX != 0 )
    {
        ...
        if ( GetTransaction(txid,tx,Params().GetConsensus(),hashBlock) == 0 )
        {
...
int32_t komodo_importaddress(const std::string &addr, const std::string &strLabel = "")
{
    ...
    if (pwallet) 
    {
        // importaddress(*pwallet, addr) - test/util.cpp
...
int32_t komodo_checkpoint(int32_t *notarized_heightp,int32_t nHeight,uint256 hash)
{
    ...
    *notarized_heightp = notarized_height;
    if ( notarized_height >= 0 && notarized_height <= pindex->nHeight && (notary= LookupBlockIndex(notarized_hash)) != 0 )
    {
